...
fn m2_fixture_has_valid_hash_manifest_lock_and_dfa() {
    ...
    pack.validate_canonical_metadata(limits).expect("manifest must bind lock graph");
    ...
    pack.validate_canonical_dependencies(limits, &[DEPENDENCY]).expect("exact dependency hash must satisfy lock");
...
fn tokenizer_manifest_identity_is_canonical() {
    ...
    let model = PackHash([
        0x69,0x86,0x5a,0xf9,0x7c,0x99,0x56,0x50,0x81,0xf3,0x93,0x2f,0x1b,0x35,0x38,0x4d,
        0x82,0x86,0x69,0xf3,0x98,0x1b,0xb7,0x96,0xdf,0x11,0x59,0x10,0x65,0xc7,0x49,0x86,
    ]);
    let resource = PackHash([
        0x9d,0xa0,0x4b,0x5f,0x24,0xe6,0xb0,0xac,0x26,0x4a,0xeb,0x71,0x4b,0x3f,0x6a,0xa5,
        0x96,0x33,0xac,0x49,0x59,0xda,0xf6,0xd1,0xdb,0x5c,0x8d,0x96,0x16,0xd5,0x27,0x94,
    ]);
    let packs = [
        ManifestPackRef { role: PackRole::Unicode, ordinal: 0, content_hash: unicode },
        ManifestPackRef { role: PackRole::Model, ordinal: 0, content_hash: model },
    ];
    let manifest = TokenizerManifestV1::new(1,1,1,1,1,1,0,resource,&packs,PackValidationLimits::DEFAULT).expect("manifest");
    assert_eq!(manifest.identity().0, [
        0xc6,0xfc,0x3e,0x21,0x7d,0xff,0x6a,0xef,0x17,0xd5,0x88,0xa9,0xed,0x7c,0x12,0xbf,
        0x4d,0xb1,0xb7,0xfc,0xc2,0x93,0x82,0x92,0xf0,0xa2,0x86,0x71,0x9e,0xc6,0x27,0x64,
    ]);
}
