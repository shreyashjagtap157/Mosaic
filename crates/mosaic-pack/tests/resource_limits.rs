...
fn outer_pack_limits_fail_before_execution() {
    ...
    let too_small = PackValidationLimits {
        max_file_bytes: 100,
        ..PackValidationLimits::DEFAULT
    };
    assert!(matches!(
        PackView::parse(bytes, too_small),
        Err(PackError::ResourceLimitExceeded)
    ));

    let too_few_sections = PackValidationLimits {
        max_sections: 2,
        ..PackValidationLimits::DEFAULT
    };
    assert!(matches!(
        PackView::parse(bytes, too_few_sections),
        Err(PackError::ResourceLimitExceeded)
    ));
}
...
fn dependency_and_dfa_limits_are_enforced() {
    ...
    let no_dependencies = PackValidationLimits {
        max_dependencies: 0,
        ..PackValidationLimits::DEFAULT
    };
    assert_eq!(
        pack.validate_canonical_metadata(no_dependencies),
        Err(PackError::ResourceLimitExceeded)
    );

    let one_state = PackValidationLimits {
        max_dfa_states: 1,
        ..PackValidationLimits::DEFAULT
    };
    assert!(matches!(
...
fn tokenizer_manifest_pack_count_is_bounded() {
    ...
    let packs = [
        ...
        ManifestPackRef {
            role: PackRole::Model,
            ordinal: 0,
            content_hash: PackHash([2; 32]),
        },
    ];
    let limits = PackValidationLimits {
