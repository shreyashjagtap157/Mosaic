...
int main(int argc,char**argv){
    ...
    for(size_t i=0;i<sizeof languages/sizeof languages[0];++i)if(!reject_language(argv[1],tok,languages[i])){mosaic_tokenizer_free(tok);return 7;}
    if(mosaic_tokenizer_language_count(tok)!=0){mosaic_tokenizer_free(tok);return 8;}
    mosaic_tokenizer_free(tok);
    puts("OK malformed model=12 unicode=9 language=10");return 0;
}
