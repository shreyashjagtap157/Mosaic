...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        py_script=temp/'python-smoke.py';py_script.write_text("from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer\nimport json\nimport os\nimport sys\nimport threading\nfrom mosaic import MosaicdClient, Tokenizer, BatchExecutor\nclass H(BaseHTTPRequestHandler):\n    def do_GET(self):\n        if self.path == '/openapi.json':\n            self._send(200, {'openapi':'3.1.0','components':{'schemas':{'ServiceProfile':{},'MetricsResponse':{}}}})\n        elif self.path == '/v1/version':\n            self._send(200, {'service_api':1,'service_profile':{'low_memory':False,'executor_workers':4,'executor_queue':256,'max_concurrency':32,'max_request_bytes':8388608,'max_decode_ids':4000000,'max_batch_items':4096,'max_batch_bytes':67108864,'max_stream_sessions':1024,'stream_pending_bytes':1048576,'stream_idle_seconds':300.0}})\n        elif self.path == '/v1/config':\n            self._send(200, {'service_api':1,'host':'127.0.0.1','port':8787,'service_profile':{'low_memory':False,'executor_workers':4,'executor_queue':256,'max_concurrency':32,'max_request_bytes':8388608,'max_decode_ids':4000000,'max_batch_items':4096,'max_batch_bytes':67108864,'max_stream_sessions':1024,'stream_pending_bytes':1048576,'stream_idle_seconds':300.0}})\n        else:\n            self._send(404, {'error':{'code':'not_found','message':'missing'}})\n    def do_POST(self):\n        if self.path == '/v1/encode': self._send(200, {'ids':[1,2,3]})\n        elif self.path == '/v1/decode': self._send(200, {'data_base64':'aGVsbG8='})\n        elif self.path == '/v1/encode-batch': self._send(200, {'results':[{'status':0,'ids':[1]},{'status':0,'ids':[2]}]})\n        else: self._send(404, {'error':{'code':'not_found','message':'missing'}})\n    def _send(self, code, payload):\n        body=json.dumps(payload).encode('utf-8')\n        self.send_response(code)\n        self.send_header('Content-Type','application/json')\n        self.send_header('Content-Length',str(len(body)))\n        self.end_headers()\n        self.wfile.write(body)\n    def log_message(self, fmt, *args):\n        pass\nroot=sys.argv[1]\nserver=ThreadingHTTPServer(('127.0.0.1',0), H)\nthread=threading.Thread(target=server.serve_forever, daemon=True)\nthread.start()\ntry:\n    client=MosaicdClient(f'http://127.0.0.1:{server.server_address[1]}')\n    assert client.openapi()['openapi']=='3.1.0'\n    assert client.version()['service_api']==1\n    assert client.config()['host']=='127.0.0.1'\n    assert client.encode(b'abc')['ids']==[1,2,3]\n    assert client.decode([1,2,3])['data_base64']=='aGVsbG8='\n    assert len(client.encode_batch([b'hello', b'world'])['results']) == 2\n    with Tokenizer(root+'/share/mosaic/packs/model-v2.mpack',root+'/share/mosaic/packs/unicode17-v1.mpack',library_path=root+'/lib/libmosaic.so') as t:\n        data=bytes(range(256))*2\n        ids=t.encode(data)\n        assert t.decode(ids)==data\n        t.seal()\n        with BatchExecutor(worker_count=2,queue_capacity=2,max_batch_items=8,max_total_input_bytes=1024,library_path=root+'/lib/libmosaic.so') as ex:\n            rs=ex.encode(t,[b'hello',b'world'])\n            assert len(rs)==2 and all(r.status==0 and r.ids for r in rs)\nfinally:\n    server.shutdown(); server.server_close()\n", encoding="utf-8")
