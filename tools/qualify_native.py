#!/usr/bin/env python3
"""Release-qualify the executable native Mosaic v0.1 tokenizer on this host."""
...
def main()->int:
    ...
    gates=[
        ...
        ['tools/generate_m3_malformed.py','--check'],['tools/build_unicode17_pack.py','--check'],
    ...
    for script in ['validate_c_reference.py','validate_c_malformed.py','validate_c_unicode.py','validate_c_unicode_malformed.py','validate_c_api.py']:
        run(py+['tools/'+script])
    ...
    clang_api=os.environ.copy();clang_api['MOSAIC_LIB']=str(clang/'libmosaic.so');run(py+['tools/validate_c_api.py'],clang_api)
    ...
    elapsed_s,rss_kb=map(float,timefile.read_text().split());throughput=(n/(1024*1024))/max(elapsed_s,1e-9)
    if throughput < 20.0: raise SystemExit(f'FAIL: throughput floor: {throughput:.1f} MiB/s < 20')
    if rss_kb > 131072: raise SystemExit(f'FAIL: RSS floor: {rss_kb:.0f} KiB > 131072')
    ...
    print('PASS: Mosaic native v0.1 release qualification completed')
