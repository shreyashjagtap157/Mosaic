...
def main()->int:
    ...
    for d in ['bin','lib','include','share/mosaic/packs/language','share/mosaic','share/pkgconfig','docs']:(stage/d).mkdir(parents=True,exist_ok=True)
    for src,dst in [(ROOT/'build/mosaic-tokenizer',stage/'bin/mosaic-tokenizer'),(ROOT/'build/libmosaic.so',stage/'lib/libmosaic.so'),(ROOT/'build/libmosaic.a',stage/'lib/libmosaic.a'),(ROOT/'native/include/mosaic.h',stage/'include/mosaic.h'),(MODEL,stage/'share/mosaic/packs/model-v2.mpack'),(UNICODE,stage/'share/mosaic/packs/unicode17-v1.mpack'),(ROOT/'README.md',stage/'README.md')]:shutil.copy2(src,dst)
    ...
    manifest={'release':'Mosaic Tokenizer','version':VERSION,'platform':tag,'c_api':'0.2.0','tokenizer_fingerprint_sha256':fingerprint,'reference_language_fingerprint_sha256':language_fingerprint,'model_pack':{'file':'model-v2.mpack','sha256':sha(MODEL)},'unicode_pack':{'file':'unicode17-v1.mpack','sha256':sha(UNICODE),'unicode_version':'17.0.0'},'language_packs':{t:{'file':f'language/{t}-v1.mpack','sha256':sha(p)} for t,p in LANGUAGES.items()},'artifacts':{}}
    ...
    print(json.dumps({'stage':str(stage),'archive':str(archive),'archive_sha256':sha(archive),'fingerprint':fingerprint,'language_fingerprint':language_fingerprint},indent=2));return 0
